        ...
        <div className="flex flex-col h-full lg:w-[70%] w-[90%] mx-auto my-10">
            <div className="flex flex-col items-center justify-center mt-10 mb-10 gap-4">
                ...
                <p className='text-center text-base text-black font-normal'>
                    ...
                    platea himenaeos neque volutpat bibendum eros, donec rutrum odio in pellentesque venenatis condimentum ut.
                    Suscipit himenaeos porta orci vitae viverra ipsum dolor sit amet consectetur adipiscing elit etiam habita
                </p>
            </div>
            <div className='flex flex-wrap gap-5 md:justify-center'>
                ...
                <div className='flex items-start gap-2'>
                    ...
                    <p className='w-[121px]'>Lo que piensan las personas de <strong>derecha</strong></p>
                </div>
                <div className='flex items-start gap-2'>
            ...
            <div className="scatter-chart mt-3">
                {
                    testComparison.map((item, index) => (
                        <div key={index} className="flex justify-around items-center h-[94px] gap-4 mx-5">
                            <div className="text-right w-[30px]">
                                {index + 1}
                            </div>
                            <div className="flex items-center relative w-[100%] h-[100%] border-t-[2px] border-b-[1px] border-solid border-slate-400">
...

                                {
                                    (item.value < item.investigation)
                                        ? <div className="absolute z-[300] h-[25px] bg-gray-300 rounded-[27px]" style={{ left: `calc(${item.value}% - 12px)`, right: `calc((100% - ${item.investigation}%) - 12px)` }}></div>
                                        : <div className="absolute z-[300] h-[25px] bg-gray-300 rounded-[27px]" style={{ left: `calc(${item.investigation}% - 12px)`, right: `calc((100% - ${item.value}%) - 12px)` }}></div>
                                }
                                ...
                                <div
                                    ...
                                    style={{ left: `${item.value > item.investigation ? item.value : item.investigation}%` }}
                                ></div>
                                <div
                                    ...
                                    style={{ left: `${item.value > item.investigation ? item.investigation : item.value}%` }}
                                ></div>
                ...
                <div className='flex justify-between mt-2 mx-[50px]'>
